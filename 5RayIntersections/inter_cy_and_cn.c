...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/25 12:29:25 by jbergfel          #+#    #+#             */
/*   Updated: 2025/03/25 13:02:55 by jbergfel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_i_cone(t_intersection **l, t_object o, t_ray r, double *t0t1)
{
	double	y0y1[2];
	...
	if (o.minimum < y0y1[0] && y0y1[0] < o.maximum)
		add_intersection(l, intersection(o, t0t1[0]));
	if (o.minimum < y0y1[1] && y0y1[1] < o.maximum)
		add_intersection(l, intersection(o, t0t1[1]));
}
...
void	intersect_cone(t_intersection **list, t_object o, t_ray r)
{
	...
	intersect_caps(list, o, r);
	abc[0] = pow(r.direction.x, 2) - pow(r.direction.y,
			2) + pow(r.direction.z, 2);
	abc[1] = 2 * (r.origin.x * r.direction.x - r.origin.y
			* r.direction.y + r.origin.z * r.direction.z);

...
/*   Updated: 2025/03/25 15:41:11 by jbergfel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	valid_cone(char **split, t_point *p, t_vector *n, t_object *n_cy, void *mlx)
{
	if (!valid_line_count(split, 13)
		...
		|| !get_reflective(split[7], split[8], &n_cy->material.reflective,
			&n_cy->material.refractive_index)
		|| !get_pattern_cone(split, &n_cy->material.pattern, mlx))
		return (0);
	return (1);
}
...
int	cone_parse(t_world *w, char *line, void *mlx)
{
	...
	if (!valid_cone(splited, &p, &n, &n_cn, mlx))
	{

...
/*   Updated: 2025/03/25 13:48:49 by jazevedo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	valid_plane(char **splited, t_point *p, t_vector *n, t_object *n_pl, void *mlx)
{
	if (!valid_line_count(splited, 11)
		...
		|| !get_color(splited[3], &n_pl->material.color)
                ...
                || !get_reflective(splited[5], splited[6],
                        &n_pl->material.reflective,
                        &n_pl->material.refractive_index)
                || !get_pattern_plane(splited, &n_pl->material.pattern, mlx))
		return (0);
	return (1);
}
...
int	plane_parse(t_world *w, char *line, void *mlx)
{
	...
	if (!valid_plane(splited, &p, &n, &n_pl, mlx))
	{

...
/*   Updated: 2025/03/18 21:57:02 by jbergfel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cam_parse(t_world *w, char *line)
{
	...
	if (!valid_line_count(line_split, 4) || !valid_has_cam(w) || !get_coords(line_split[1], &cam_position) || !get_dir(line_split[2], &cam_direction) || !get_int(line_split[3], &fov))
	{
		printf("cam_parse ko\n");
		//free split
		return (0);

...
void	render_sphere(t_canvas *canvas)
{
	...
	while (xy[1] < canvas->height)
	{
		...
		while (xy[0] < canvas->width)
		{
			...
			if (h.t > 0)
			{
				point_of_intersection = position(r, h.t);
				normalv = normal_at(sphere, point_of_intersection);
				eyev = inverse_tuple(r.direction);
				pixel_color = lighting(sphere.material, light, point_of_intersection, eyev, normalv);

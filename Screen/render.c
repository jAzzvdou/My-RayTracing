...
void render_scene(t_minilibx *libx)
{
	...
	t_object glass = glass_sphere();
	glass.transformed = translation(0, 1, 0); // No centro
	...
	t_world w = world();
	t_camera cam = camera(400, 200, M_PI / 3);
...
void render_tests(t_minilibx *libx)
{
	...
	t_object a = new_object(PL);
	a.material.amb = 1.0;
	a.material.pattern = new_pattern(STRIPE, color(1, 1, 1), color(0, 0, 0));
	add_object(&w.object, a);
	...
	add_intersection(&xs, intersection(a, -0.9899));
	add_intersection(&xs, intersection(b, -0.4899));
	add_intersection(&xs, intersection(b, 0.4899));
	add_intersection(&xs, intersection(a, 0.9899));

	t_comps comps = prepare_computations(xs[2], r, xs);

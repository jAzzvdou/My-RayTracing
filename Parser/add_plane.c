...
t_plane	*fill_plane(char **split)
{
	...
	plane->coord.x = ft_atod(tmp[0]);
	plane->coord.y = ft_atod(tmp[1]);
	plane->coord.z = ft_atod(tmp[2]);
	tmp = memcard(tmp, VECTOR, FREE, 0);
	tmp = splitline(split[2], ',');
	plane->orientation.x = ft_atod(tmp[0]);
	plane->orientation.y = ft_atod(tmp[1]);
	plane->orientation.z = ft_atod(tmp[2]);
	tmp = memcard(tmp, VECTOR, FREE, 0);

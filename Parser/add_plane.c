...
t_plane	*fill_plane(char **split)
{
	...
	plane->coord[0] = my_atod(tmp[0]);
	plane->coord[1] = my_atod(tmp[1]);
	plane->coord[2] = my_atod(tmp[2]);
	tmp = memcard(tmp, VECTOR, FREE, 0);
	tmp = splitline(split[2], ',');
	plane->nvector[0] = my_atod(tmp[0]);
	plane->nvector[1] = my_atod(tmp[1]);
	plane->nvector[2] = my_atod(tmp[2]);
	tmp = memcard(tmp, VECTOR, FREE, 0);

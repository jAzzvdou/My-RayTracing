...
t_plane	*fill_plane(char **split)
{
	...
	plane->coord.x = my_atod(tmp[0]);
	plane->coord.y = my_atod(tmp[1]);
	plane->coord.z = my_atod(tmp[2]);
	tmp = memcard(tmp, VECTOR, FREE, 0);
	tmp = splitline(split[2], ',');
	plane->orientation.x = my_atod(tmp[0]);
	plane->orientation.y = my_atod(tmp[1]);
	plane->orientation.z = my_atod(tmp[2]);
	tmp = memcard(tmp, VECTOR, FREE, 0);
	tmp = splitline(split[3], ',');
	plane->rgb[0] = my_atoi(tmp[0]);
	plane->rgb[1] = my_atoi(tmp[1]);
	plane->rgb[2] = my_atoi(tmp[2]);

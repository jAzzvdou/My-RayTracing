...
t_cylinder	*fill_cylinder(char **split)
{
	...
	cylinder->coord.x = ft_atod(tmp[0]);
	cylinder->coord.y = ft_atod(tmp[1]);
	cylinder->coord.z = ft_atod(tmp[2]);
	tmp = memcard(tmp, VECTOR, FREE, 0);
	tmp = splitline(split[2], ',');
	cylinder->orientation.x = ft_atod(tmp[0]);
	cylinder->orientation.y = ft_atod(tmp[1]);
	cylinder->orientation.z = ft_atod(tmp[2]);
	tmp = memcard(tmp, VECTOR, FREE, 0);
